...
#if defined(NDEBUG)
# error "Magutcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xab9f507d8414881c6f7e52bfcff69399fdb6119c58ad19a1035c75c56b268009");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Magutcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Magutcoin Signed Message:\n";
...
{
    // Magutcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Magutcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Magutcoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Magutcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Magutcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Magutcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x596727536ab9ce89de28d0aa8a4fd81210c421ed79e8c3210fec6ac4e8c961ea");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("040186a6968148255f3bdf929010f69a0b76a4db5f519c291c1bcd77d1f4d871d4ece54fbbe89c6d02d5bd9978564165a0927ac02d5b2eee109c378b8f5b4a44e1") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1574243845;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084568212;

        if (fTestNet)
        {
            block.nTime    = 1574243872;
            block.nNonce   = 387583952;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xe7cec187d5cc7b329a0d4be2a6d97147d7671ae9b821051db6eb8fc55d01c35c"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf2, 0xc3, 0xb5, 0xd8 }; // Magutcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MagutcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MagutcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MagutcoinMiner(CWallet *pwallet)
{
    printf("MagutcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("magutcoin-miner");
    ...
    try { loop {
        ...
        printf("Running MagutcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MagutcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MagutcoinMiner, pwallet));
}
